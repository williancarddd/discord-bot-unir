...
export default new Command({
  ...
  run: async ({client, interaction}) => {
    ...
    if(trackNumber > queue.tracks.length) {
      return await interaction.editReply({content: `The track number must be between 1 and ${queue.tracks.length}`});
    }
    queue.skipTo(trackNumber - 1);
    return await interaction.editReply({content: `Skipped to track number ${trackNumber}`});
  }
});